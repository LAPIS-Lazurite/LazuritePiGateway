...
#ifndef	_SPI_BP3596_H_
#define	_SPI_BP3596_H_
 ...
 * data: 書き込むデータ
 * length: dataのバイト数 (SPI_BUFFERSIZE-1 が最大値)
 * 戻り値: 正数=書き込んだデータバイト数, 負数=異常終了
 */
extern int bp_spi_write(uint8_t bank, uint8_t address, const uint8_t *data, uint8_t length);

/* SPI データ読み出し処理
 * bank: データを読み出すBANK番号
 * address: データを読み出すアドレス
 * data: 読み出しデータの収納領域
 * size: dataのバイト数 (SPI_BUFFERSIZE-1 が最大値)
 * 戻り値: 正数=読み出したデータバイト数, 負数=異常終了
 */
extern int bp_spi_read(uint8_t bank, uint8_t address, uint8_t *data, uint8_t size);

#endif	// _SPI_BP3596_H_
