...
static int expired  = 10;               // Send Expired
module_param(expired, int, S_IRUGO);
...
const static int mac_func_data(void)
{
	...
	if(addr_comp(&macParam.rxHdr.rx_addr) == true)
	{
		...
		else		//Ack is not requested
		{
			...
			DEBUGONDISPLAY(MODE_MAC_DEBUG, printk("no ack\n"));
		}
	}
	else
	{
...
int mac_send(const uint8_t *data,uint16_t len)
{
	...
	g_timer.data = 0;
	// 2015.08.27 Eiichi Saito
	g_timer.expires = jiffies + HZ/expired;
	...
	if(macParam.txHdr.ack_req == 1)
	{
		...
		add_timer(&g_timer);
		//finish_flag = 0;
		phy.send(macParam.txHdr.raw.data,macParam.txHdr.raw.len);
		finish_flag2 = 0;
		do
		{
